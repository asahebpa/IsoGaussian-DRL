...
def compute_losslandscape_metrics(agent, obs, returns, actions, power_iters=20, trace_samples=50, eps=1e-3, prefex=None):
    """
    Returns:
      lambda_max : largest Hessian eigenvalue (scalar)
      lambda_min : smallest Hessian eigenvalue (scalar)
      trace_est  : Hutchinson's estimator of Tr(H) (scalar)
      condition_number : estimate of condition number (lambda_max / |lambda_min|)
    """
    ...
    # Compute largest eigenvalue (lambda_max) using power iteration
    ...
    # Compute smallest eigenvalue (lambda_min) using inverse power iteration
    # We compute the largest eigenvalue of (H + shift*I)^{-1} and then invert
    shift = abs(lambda_max) + eps  # Shift to make H + shift*I positive definite
    
    def shifted_inverse_hvp(v):
        # Solve (H + shift*I)w = v for w using conjugate gradient
        # This is equivalent to computing (H + shift*I)^{-1} v
        w = torch.zeros_like(v)
        r = v.clone()
        p = r.clone()
        rsold = torch.dot(r, r)
        
        for _ in range(min(50, num_params)):  # CG iterations
            Hp = hvp(p)
            Ap = Hp + shift * p  # (H + shift*I)p
    ...
    v_min = torch.randn(num_params, device=flat_params.device)
    v_min /= v_min.norm()
    for _ in range(power_iters):
        v_min = shifted_inverse_hvp(v_min)
        v_min /= v_min.norm()
    
    # Compute eigenvalue of shifted inverse operator
    shifted_inv_v = shifted_inverse_hvp(v_min)
    lambda_shifted_inv = torch.dot(v_min, shifted_inv_v).item()
    
    # Convert back to eigenvalue of H
    lambda_min = (1.0 / lambda_shifted_inv) - shift
    
    # Compute condition number
    condition_number = abs(lambda_max) / max(abs(lambda_min), eps)
    ...
    return {
        f"loss_landscape/lambda_max_{prefex}": lambda_max,
        f"loss_landscape/lambda_min_{prefex}": lambda_min,
        f"loss_landscape/trace_est_{prefex}": trace_est,
        f"loss_landscape/condition_number_{prefex}": condition_number,
    }
