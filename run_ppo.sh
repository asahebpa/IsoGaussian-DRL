...
for env in "${environments[@]}"; do
    for scheme in "${schemes[@]}"; do
        ...
        for seed in "${seeds[@]}"; do
            job_name="PPO_${env}_${suffix}_seed${seed}"
            output_file="${OUTPUT_DIR}/${job_name}.txt"

            # Build command with optimizer and sigreg_lambda
            cmd="python ./ppo.py --mlp_type="default"  --mlp_depth="medium" --env_id=$env --seed=$seed --optimizer=$opt --sigreg_lambda=$sigreg_lambda"
